...
# 結果の画像に描画
for idx, (pair, dist) in enumerate(closest_pairs_distance):
    label_0_bbox, label_1_bbox = pair
    ...
    cv2.rectangle(cv_image, 
                  (int(label_0_bbox[0]), int(label_0_bbox[1])), 
                  (int(label_0_bbox[2]), int(label_0_bbox[3])), 
                  color, 2)  # 色と太さを指定

    # Label 1 の矩形
    cv2.rectangle(cv_image, 
                  (int(label_1_bbox[0]), int(label_1_bbox[1])), 
                  (int(label_1_bbox[2]), int(label_1_bbox[3])), 
                  color, 2)  # 色と太さを指定

    # ペアの重心位置にテキストを追加（任意）
    center_0 = ((label_0_bbox[0] + label_0_bbox[2]) / 2, (label_0_bbox[1] + label_0_bbox[3]) / 2)
    center_1 = ((label_1_bbox[0] + label_1_bbox[2]) / 2, (label_1_bbox[1] + label_1_bbox[3]) / 2)
    cv2.putText(cv_image, f"{dist:.2f}", 
                (int((center_0[0] + center_1[0]) / 2), int((center_0[1] + center_1[1]) / 2)),
...
# 画像の描画関数
def draw_bboxes_with_intersection(cv_image, label_0_bboxes, label_1_bboxes, closest_pairs_share_surface):
    for label1_bbox, label0_bbox, area in closest_pairs_share_surface:
        if area > 0:
            # label0とlabel1のバウンディングボックスを描画
            cv2.rectangle(cv_image, (label0_bbox[0], label0_bbox[1]), (label0_bbox[2], label0_bbox[3]), (0, 255, 0), 2)  # 緑色
            cv2.rectangle(cv_image, (label1_bbox[0], label1_bbox[1]), (label1_bbox[2], label1_bbox[3]), (0, 0, 255), 2)  # 赤色

            # 交差領域を描画
            _, inter_bbox = intersection_area(label0_bbox, label1_bbox)
            if inter_bbox:
                inter_left, inter_top, inter_right, inter_bottom = inter_bbox
                cv2.rectangle(cv_image, (inter_left, inter_top), (inter_right, inter_bottom), (255, 0, 0), 2)  # 青色で交差部分
...
#一致するペアを見つける 
def find_closest_pairs(closest_pairs_distance, closest_pairs_share_surface):
    closet_pairs = []
    ...
    for ((label_0_bbox_1, label_1_bbox_1), dist) in closest_pairs_distance:
        for label_1_bbox_2, label_0_bbox_2, area in closest_pairs_share_surface:
            ...
            if label_0_bbox_1 == label_0_bbox_2 and label_1_bbox_1 == label_1_bbox_2:
                # 一致するペアを格納
                closet_pairs.append((label_1_bbox_1, label_0_bbox_1, dist, area))    
    return closet_pairs

# 両方のリストを一致する組み合わせを探して統合
closet_pairs = find_closest_pairs(closest_pairs_distance, closest_pairs_share_surface)

# 結果を表示
print("一致するペア:")
for pair in closet_pairs:
    print(pair)

# 結果の画像を表示
cv2.imshow("Image with Intersection Highlight", cv_image_with_bboxes)
cv2.waitKey(0)
cv2.destroyAllWindows()

# 画像の表示
cv2.imshow("Image with Closest Pairs", cv_image)
cv2.waitKey(0)
cv2.destroyAllWindows()
